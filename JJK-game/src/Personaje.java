...
public abstract class Personaje {
    ...
    public void ataqueBasico(Personaje objetivo) {
        // ARREGLO: null-check para evitar NPE si todos los rivales están muertos
        if (objetivo == null) {
            System.out.println("  No hay rival disponible para atacar.");
            return;
        }
        System.out.println("\n>>> " + this.nombre + " lanza un Ataque Fisico Comun.");
        int danio = 30;
        if (puedeUsarEspeciales() && Math.random() < 0.05) {
            System.out.println("DESTELLO NEGRO! (Black Flash)"); danio = (int)(danio * 2.5);
        }
    ...
    public void usarHabilidad(int indice, Personaje objetivo) throws Exception {
        ...
        // ARREGLO: null-check general antes de marcar/dañar al objetivo
        boolean golpeEnergetico = h.getCosteEnergia() > 0 || this.portadorHerramienta;
        if (objetivo != null) objetivo.marcarUltimoGolpe(golpeEnergetico);

        if (objetivo != null && h.getNombre().equals("Tajo de Exterminio") && objetivo instanceof Maldicion) {
            System.out.println("\n>>> " + this.nombre + " usa: " + h.getNombre());
            System.out.println("La Espada del Exterminio purifica al instante!");
            objetivo.vida = 0; return;
        }
        if (objetivo != null && h.getNombre().contains("JACOB") && h.getNombre().contains("ANIQUILACI") && objetivo instanceof Maldicion) {
            System.out.println("\n>>> " + this.nombre + " usa: " + h.getNombre());
        ...
        if (this.dominioActivo) danioFinal = (int)(danioFinal * 1.3);
        ...
        if (h.isFisico() && puedeUsarEspeciales()) {
            ...
            if (bf) { System.out.println("DESTELLO NEGRO! Las chispas oscuras vuelan."); danioFinal = (int)(danioFinal * 2.5); }
        }
        ...
        if (nom.contains("VAC") && nom.contains("INFINITO")) {
            ...
            if (objetivo != null) System.out.println("Singularidad liberada sobre " + objetivo.getNombre() + ".");
        }
        if (danioFinal > 0 && objetivo != null) objetivo.recibirDanio(danioFinal);
    }
    ...
    public void prepararTurno() {
        ...
        // ARREGLO: el estado DEBILITADO (negativo) también debe decrementar cada turno.
        // Sin esta línea, el debilitamiento era permanente una vez aplicado.
        if      (turnosPotenciado > 0) turnosPotenciado--;
        else if (turnosPotenciado < 0) turnosPotenciado++;
    }
    ...
    // Getter para no exponer el campo protected fuera de la jerarquía
    public boolean estaInmovilizado()            { return turnosInmovilizado > 0; }
    ...
    // Marca personajes invocables (no seleccionables en el menú): Mahoraga, Naoya (Maldicion)...
    protected boolean especial = false;

    public boolean isEspecial()        { return especial; }
    public void    setEspecial(boolean b) { this.especial = b; }

    /**
     * Crea una copia fresca de este personaje con stats y habilidades originales.
     * Habilidad es inmutable, se puede compartir entre instancias sin riesgo.
     */
    public Personaje clonar() {
        Personaje copia = (this instanceof Maldicion)
                ? new Maldicion(this.nombre, this.maxVida, this.maxEnergia)
                : new Hechicero(this.nombre, this.maxVida, this.maxEnergia);
        for (Habilidad h : this.habilidades) copia.addHabilidad(h);
        copia.setCuracionDisponible(this.curacionDisponible);
        copia.setPortadorHerramienta(this.portadorHerramienta);
        copia.setSinEnergiaMaldita(this.sinEnergiaMaldita);
        copia.setEspecial(this.especial);
        return copia;
    }
}